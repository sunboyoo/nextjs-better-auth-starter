...
interface UserRolePermissionTreeDisplayProps {
    permissions: Record<string, string[]>;
}

export function UserRolePermissionTreeDisplay({ permissions }: UserRolePermissionTreeDisplayProps) {
                    ...
                    {resourceKeys.map((resource) => (
                        <div key={resource}>
                            ...
                            <div className="font-medium text-muted-foreground/90 mb-0.5 flex items-center gap-1 capitalize">
                                <span className="w-1.5 h-1.5 rounded-full bg-purple-500/50 inline-block mb-[1px]" />
                                {resource}
                            </div>
